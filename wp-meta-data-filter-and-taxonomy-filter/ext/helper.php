...
final class MDTF_HELPER {
		...
		if (isset($data[2]) AND !empty($data[2])) {
			...
			if ($data[0] == 'LIKE') {
				foreach ($words as $key => $word) {
					$words[$key] = $wpdb->prepare(" post_title LIKE %s ", '%' . $wpdb->esc_like($word) . '%');
				}
			} else {
				foreach ($words as $key => $word) {
					$words[$key] = $wpdb->prepare(" post_title = %s ", $word);
				}
			}
	...
	public static function mdf_post_content_filter($where = '') {
		...
		if (isset($data[2]) AND !empty($data[2])) {
			...
			if ($data[0] == 'LIKE') {
				foreach ($words as $key => $word) {
					$words[$key] = $wpdb->prepare(" post_content LIKE %s ", '%' . $wpdb->esc_like($word) . '%');
				}
			} else {
				foreach ($words as $key => $word) {
					$words[$key] = $wpdb->prepare(" post_content = %s ", $word);
				}
			}
		...
		if (isset($data[2]) AND !empty($data[2])) {
			...
			if ($data[0] == 'LIKE') {
				foreach ($words as $key => $word) {
					...
					$words[$key] = $wpdb->prepare(" post_content LIKE %s OR post_title LIKE %s OR post_excerpt LIKE %s", $escaped, $escaped, $escaped);
				}
			} else {
				foreach ($words as $key => $word) {
					$words[$key] = $wpdb->prepare(" post_content = %s OR post_title = %s OR post_excerpt = %s", $word, $word, $word);
				}
			}
	...
	public static function mdf_post_title_and_content_filter($where = '') {
		...
		if (isset($data[2]) AND !empty($data[2])) {
			...
			if ($data[0] == 'LIKE') {
				foreach ($words as $key => $word) {
					$escaped = '%' . $wpdb->esc_like($word) . '%';
					$words[$key] = $wpdb->prepare(" post_content LIKE %s AND post_title LIKE %s", $escaped, $escaped);
				}
			} else {
				foreach ($words as $key => $word) {
					$words[$key] = $wpdb->prepare(" post_content = %s AND post_title = %s", $word, $word);
				}
			}
	...
	public static function get_filtered_price() {
		...
		if (apply_filters('mdf_woo_price_dinamic_recount', false)) {
			...
			$args1 = self::sanitize_array_r($_REQUEST['meta_data_filter_args']);
